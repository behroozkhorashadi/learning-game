...
import type { ZombieArenaBrain } from './zombieArenaTypes'

/**
 * The reusable "zombie box": a fixed-camera 3D shooting-gallery speed
 * round — a question shows, four "zombies" walk toward the camera carrying
 * candidate answers, and the player zaps the one holding the correct
 * answer before any of them arrives. Everything about *what* the question
 * is and how a resolved round should be scored/logged is supplied by the
 * `brain` prop (see `zombieArenaTypes.ts`); this component owns the
 * round/session lifecycle, hit resolution, weapon/roster rendering, HUD,
 * and audio, identically for every game built on it.
 *
...
// exactly four CarrierOptions from the brain's mapToOptions) — the roster
// matches that today. Growing the character registry past four enabled
// entries does not change this: selectSessionRoster still returns four
// uniques, chosen from whatever's enabled.
const ROSTER_SIZE = 4
const WEAPON = STARTER_BLASTER
const WRONG_HIT_SPEED_BOOST = 0.18

// A short beat where the question fills the screen before the carriers
...
function buildWaveConfig(brain: ZombieArenaBrain, item: Item): WaveConfig {
  return {
    approachMs: brain.getApproachMs(item),
...
interface Props {
  profileId: number
  onBack: () => void
  brain: ZombieArenaBrain
}

export function ZombieArena({ profileId, onBack, brain }: Props) {
  ...
  // brief "loading next question" gap between them) and stops the moment
  ...
  const fetchItem = useCallback(async () => {
    setError(null)
    try {
      const res = await fetch(`/api/items/next?profile_id=${profileId}&game_id=${brain.gameId}&session_id=${sessionId}`)
      if (!res.ok) throw new Error(`GET /api/items/next -> ${res.status}`)
      const next: Item = await res.json()
      setItem(next)
      configRef.current = buildWaveConfig(brain, next)
      setWave(null)
  ...
  // Intro beat: show the full question, then spawn the wave.
  useEffect(() => {
    if (phase !== 'intro' || !item) return
    const timer = setTimeout(() => {
      const options = brain.mapToOptions(item)
  ...
  useEffect(() => {
    ...
    if (wave.outcome === 'solved') {
      ...
      setLiveMessage(`Correct! ${summary}`)
      const timer = setTimeout(() => {
        setSolvedItems((prev) => {
          const nextList = [...prev, summary]
          if (nextList.length >= brain.sessionLength) {
            setPhase('complete')
            brain.onSessionEnd?.({ outcome: 'complete', solvedCount: nextList.length, sessionLength: brain.sessionLength, livesRemaining: lives })
          } else {
    ...
    brain.onQuestionResult(item, { correct: false, timeTakenMs: wave.elapsedMs, wrongShots: wave.wrongShots }, sessionId)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setLives((prev) => {
        ...
        if (remaining <= 0) {
          brain.onSessionEnd?.({ outcome: 'game_over', solvedCount: solvedItems.length, sessionLength: brain.sessionLength, livesRemaining: 0 })
        }
  ...
  function submitRating(value: number) {
    ...
    fetch('/api/ratings', {
      ...
      body: JSON.stringify({ profile_id: profileId, game_id: brain.gameId, scale: 'stars_1_5', value }),
    ...
    <div style={{ width: '100%', height: '100vh', boxSizing: 'border-box', background: 'var(--surface-app)', overflow: 'hidden', display: 'flex' }}>
      ...
      >
        ...
        {phase === 'start' && (
          <SessionStart
            eyebrow="Speed round"
            headline={brain.copy.title}
            subtitle={brain.copy.subtitle}
            sessionLength={brain.sessionLength}
            heroSrc={`/images/badges/${brain.gameId}.png`}
        ...
        {isGamePhase && item && (
          ...
          >
            ...
            <div style={{ position: 'absolute', top: 16, left: 16, right: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 200, pointerEvents: 'none' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: 40, padding: '0 14px', borderRadius: 9999, background: 'rgba(255,255,255,0.9)', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 16, color: '#2A2E37' }}>
                  {solvedItems.length} / {brain.sessionLength}
                </div>
              ...
              {(wave?.carriers ?? []).map((carrier) =>
                carrier.status === 'active' ? (
                  <button
                    ...
                    aria-label={`Zap the zombie carrying ${carrier.label}`}
                    style={{
                      height: 36,
                      padding: '0 12px',
                      borderRadius: 9999,
             ...
             * it never obscures the question, answer labels, or the
              ...
              >
                <div style={{ background: '#FFF6EA', borderRadius: 28, padding: '32px 36px', maxWidth: 420, textAlign: 'center', boxShadow: '0 22px 44px -16px rgba(0,13,51,0.4)' }}>
                  ...
                  <p style={{ color: '#8896AA', fontSize: 15, marginBottom: 6 }}>
                    Lives left: {lives} · Score: {solvedItems.length}/{brain.sessionLength}
                  </p>
        ...
        {phase === 'transitioning_to_next_wave' && <p style={{ color: 'var(--fg-tertiary)', textAlign: 'center' }}>Loading the next question…</p>}

        {phase === 'complete' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {!ratingHandled && <RatingPrompt onRate={submitRating} onDismiss={() => setRatingHandled(true)} />}
            <SessionComplete
              headline={brain.copy.sessionCompleteHeadline}
              subtitle={brain.copy.sessionCompleteSubtitle}
              badgeSrc={`/images/badges/${brain.gameId}.png`}
